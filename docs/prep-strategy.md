...
- **`predict_human_move.wdlWhitePov.draw` compares directly across positions.** When choosing between two lines for a specific practical goal (must-win, must-hold, comfortable-with-both-sides), call the tool on both positions and compare the draw share. Example: Berlin `4.0-0 Nxe4 5.Re1` returns ~50% draw at 2600 — the Italian `4.d3 d6` returns ~30% draw at the same rating. The user asking "must-win with Black" gets the Italian; the user asking "must-hold" gets the Berlin. Quantifies intuition that would otherwise be handwaved.
